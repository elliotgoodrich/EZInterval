...
// is_true<bool X> has the typedef type defined as std::true_type only if X is true
template <bool>
struct is_true;
...
// calling is_iterator<T>(0) will return std::true_type if T has T::iterator_category defined and
// std::false_type otherwise
template <typename T>
auto is_iterator(int) ->
	decltype(is_defined<decltype(std::declval<typename T::iterator_category>())>());

template <typename T>
std::false_type is_iterator(long);

// calling has_difference_type<T>(0) will return std::true_type if T has T::difference_type
// defined and std::false_type otherwise
template <typename T>
auto has_difference_type(int) ->
	decltype(is_defined<decltype(std::declval<typename T::difference_type>())>());

template <typename T>
std::false_type has_difference_type(long);

// difference_type_impl decides what difference_type should be used for direct_iterator<T>
...
struct difference_type
	: difference_type_impl<T,
		std::remove_reference<decltype(has_difference_type<T>(0))>::type::value> {
};


// calling input_compatible<T>(0) will return std::true_type if T is compatible with being a
// direct_iterator<T> which satisfies the InputIterator concepts
template <typename T>
auto input_compatible(int) ->
	decltype(
...
// calling forward_compatible<T>(0) will return std::true_type if T is compatible with being a
// direct_iterator<T> which satisfies the ForwardIterator concepts
template <typename T>
auto forward_compatible(int) ->
	decltype(
	// ForwardIterator Concepts
		typename is_true<decltype(input_compatible<T>(0))::value>::type(),
		typename is_true<std::is_default_constructible<T>::value>::type() // Default constructible
	);

template <typename T>
std::false_type forward_compatible(long);

// calling bidirectional_compatible<T>(0) will return std::true_type if T is compatible with being a
// direct_iterator<T> which satisfies the BidirectionalIterator concepts
template <typename T>
auto bidirectional_compatible(int) ->
	decltype(
	// ForwardIterator Concepts
...
// calling random_access_compatible<T>(0) will return std::true_type if T is compatible with being a
// direct_iterator<T> which satisfies the RandomAccessIterator concepts
template <typename T>
auto random_access_compatible(int) ->
	decltype(
	// BidirectionalIterator Concepts
		typename is_true<decltype(bidirectional_compatible<T>(0))::value>::type(),
	// RandomAccessIterator Concepts
		is_defined<decltype(std::declval<T&>() += std::declval<typename difference_type<T>::type>())>(),
		is_defined<decltype(std::declval<T&>() + std::declval<typename difference_type<T>::type>())>(),
		is_defined<decltype(std::declval<typename difference_type<T>::type&>() + std::declval<T&>())>(),
		is_defined<decltype(std::declval<T&>() -= std::declval<typename difference_type<T>::type>())>(),
		is_defined<decltype(std::declval<T&>() - std::declval<typename difference_type<T>::type>())>(),
...
template <typename T, bool IsIterator, bool RandomAccess, bool Bidirectional, bool Forward, bool Input>
struct iterator_tag_impl;

// iterator_tag_impl decides what iterator_category should be used for direct_iterator<T>
template <typename T, bool RandomAccess, bool Bidirectional, bool Forward, bool Input>
...
struct iterator_tag
	: iterator_tag_impl<T,
		std::remove_reference<decltype(is_iterator<T>(0))>::type::value,
		std::remove_reference<decltype(random_access_compatible<T>(0))>::type::value,
		std::remove_reference<decltype(bidirectional_compatible<T>(0))>::type::value,
		std::remove_reference<decltype(forward_compatible<T>(0))>::type::value,
		std::remove_reference<decltype(input_compatible<T>(0))>::type::value> {
};
...
public:
	...
	explicit direct_iterator(T const& value) noexcept(std::is_nothrow_copy_constructible<T>::value)
	: m_value{value} {
	}

	explicit direct_iterator(T&& value) noexcept(std::is_nothrow_move_constructible<T>::value)
...
template <typename T>
typename direct_iterator<T>::difference_type operator-(direct_iterator<T> const& lhs,
                                                       direct_iterator<T> const& rhs) {
	return *lhs - *rhs;
}
